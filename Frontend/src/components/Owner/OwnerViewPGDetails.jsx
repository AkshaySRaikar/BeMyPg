...
// const MapPg=() => {
//         const address = 'No 10, Prakruthi, 1st floor, Kommaghatta, near Srikanteshwara Temple, Bangalore';
...
//         .then(res => res.json())
//         .then(json => {
//             // Check the API response for coordinates (latitude and longitude)
...
//     // Assuming your API response gives you the coordinates
//     const lat = pgDetails?.latitude; // Replace with actual latitude
//     const lon = pgDetails?.longitude; // Replace with actual longitude
...
//             <div key={room._id.$oid || index} className="mb-4 p-4 bg-gray-900 rounded-lg shadow">
...
//             </div>
//             ))}
//             <button className='bg-slate-50' onClick={()=>{MapPg()}}>Add Your Pg To Maps</button>
//         </div>
...
const OwnerPGDetails = () => {
    ...
    const handleShowLocation = async () => {
        ...
        try {
            console.log(pgDetails.Address)
            const coordinates = await fetchCoordinates(`${pgDetails.Address},${pgDetails.City}`);
    ...
    return (
        ...
        {/* Show Location Button */}
        <button onClick={handleShowLocation} className="bg-blue-500 text-white px-4 py-2 rounded mt-4">
            Show Location
        </button>
